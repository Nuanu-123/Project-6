...
namespace MarsQA_1.Pages
{
    ...
    {
        private static IWebElement SignIntab =>  Driver.driver.FindElement(By.XPath("//A[@class='item'][text()='Sign In']"));
        ...
        public static void SigninStep()
        {
            ...
            try
            {
                ...
                WaitHelpers.WaitForElement(Driver.driver, "XPath", "//*[contains(text(),'Anusree Rakesh')]", 50);
                // check login is successfull
                var loginCheck = "test";
                loginCheck = Driver.driver.FindElement(By.XPath("//*[contains(text(),'Anusree Rakesh')]")).GetAttribute("innerText");

                if (loginCheck == "Anusree Rakesh")
                {
